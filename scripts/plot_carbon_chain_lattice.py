...
data = pd.read_csv("data/chains/carbon/lattice_scan.csv")
...
plt.title("1D Carbon Chain: Total Energy vs Lattice Constant")

plt.annotate(
    f"Minimum: a = {equilibrium_lattice:.2f} Å",
    (equilibrium_lattice, minimum_energy),
    xytext=(equilibrium_lattice + 0.015, minimum_energy + 0.04),
...
plt.savefig("figures/chains/carbon/carbon_chain_lattice_scan.png", dpi=300)
plt.show()
